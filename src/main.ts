...
async function beat() {
  ...
  if ((epoch.hours_from_start()>0.5 && epoch.hours_from_start()<1.5) || debugMode) {
    ...
    for(let inx=0;inx<pools.length;inx++) {
      ...
      if (pool.last_asked_rewards_epoch_height!=env_epoch_height)
        ...
        let result = await metaPool.call("distribute_rewards",{sp_inx_i32:inx});
        console.log(`RESULT:${yton(result)}N`)
    }

    for(let inx=0;inx<pools.length;inx++) {
      const pool = pools[inx];
      if (epoch_difference(env_epoch_height,pool.unstaked_requested_epoch_height)>=0 || debugMode) {
          //ping on the pool so it calculates rewards
          console.log(`about to call RETIEVE UNSTK FUNDS on pool[${inx}]:${JSON.stringify(pool)}`)
